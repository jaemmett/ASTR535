...
FUNCTION count_rate,radius,lambda,bandwidth,maglambda
...
;and bandwidth (bandwidth) [A].
	;CONSTANTS
		...
		h=6.6260755e-27  ;erg*s
	;FUNCTION
		k=circle_area(radius)*lambda2efficiency(lambda)*lambda2transmission(lambda)*maglambda2flambda(maglambda,lambda)/(h*c)
		lambda1=lambda-(bandwidth/2.0)
		lambda2=lambda+(bandwidth/2.0)
...
FUNCTION count_rate_int,radius,lambda_range,maglambda
;Calculates a count rate [photons/s] 
;given a telescope aperture radius (radius) [cm], central wavelength (lambda) [A]
;bandwidth (bandwidth) [A], and magnitude at the central wavelength (maglambda).
	;CONSTANTS
		c=2.99792458e18  ;A/s
		h=6.6260755e-27  ;erg*s
	;FUNCTION
		...
		for lambda=lambda_range[0],lambda_range[1],lambda_range[2] do begin
		endfor
END
...
pro q13
	...
	;radius=50 [cm], central wavelength=5500 [A],
	;bandwidth=1000 [A].
	;	rate=count_rate(50.0,5500.0,1000.0,22.0)
	;	print,'count rate [ph/s]',rate

	;Calculate the photon count rate for
	;radius=50 [cm], central wavelength=5500 [A],
	;bandwidth=1000 [A].
		rate=count_rate_int(50.0,[5000.0,6000.0,100.0],22.0)
		print,'count rate [ph/s]',rate
